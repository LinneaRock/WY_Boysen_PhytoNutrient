...
ggsave('Figures/ASLO24/top5_predictors.png',height=4.5,width=6.5,units='in',dpi=1200)


# dark theme for aslo top 6-10
bda_cyano |>
  filter(ShortName_Revised %in% c((top10 |> slice(6:10))$var)) |>
  data.frame() |>
  mutate(ShortName_Revised = factor(ShortName_Revised, levels=c(top10$var), labels=c('x','x', 'x', 'x', 'x',expression('TP'~(mg~L^-1)), expression('Schmidt Stability Index'~(J~m^-2)),'TN:TP~ratio', expression('Nitrate'~(mg~L^-1)), expression('Phosphate'~(mg~L^-1))))) |>
