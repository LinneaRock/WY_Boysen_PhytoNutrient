...
for(v in vars) {
      ...
      tryCatch({ # continues running loop, but gives error message where problem(s) are occurring
        ...
        idw_dat_tmp <- idw_dat_tmp |> mutate(month=m)
        ...
        idw_dat$month <- factor(idw_dat$month, levels=c('May','Jun','Jul','Aug','Sep','Oct'))
        ...
        p <- ggplot()+
          ...
          geom_sf(data=cyano_dat, aes(), fill='#999933', color='white', size=2.25, shape=21) +
          scale_color_gradient(paste0(v), high='grey98', low='grey2',limits=c(minlim, maxlim)) +
          theme_bw() +
          theme(axis.text.x=element_text(angle=45),
                axis.text.y=element_text(angle=45)) +
          scale_x_continuous(breaks = seq(from = -108.26, to = -108.14, by = 0.04)) +
